...
class SummaryTrainingData(models.Model):
    ticket= models.OneToOneField(JiraTicket,on_delete=models.CASCADE, null=False, blank=False)
    quality_score = models.IntegerField(null=False, blank=False) 

class DescriptionTrainingData(models.Model):
    ticket= models.OneToOneField(JiraTicket, on_delete=models.CASCADE, null=False, blank=False)
    quality_score = models.IntegerField(null=False, blank=False)

class PriorityTrainingData(models.Model):
    ticket= models.OneToOneField(JiraTicket, on_delete=models.CASCADE, null=False, blank=False)
    logical_relation_score = models.IntegerField(null=False, blank=False)
